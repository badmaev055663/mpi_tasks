...
{
    MPI_Status status;
    rand_fill_vec_int(sbuf, n);
    double t1 = MPI_Wtime();
	int ret = MPI_Sendrecv(sbuf, n, MPI_INT, pid, 0, rbuf, n, MPI_INT, pid, 0, MPI_COMM_WORLD, &status);
	if (ret) {
		printf("receive failed with code: %d\n", ret);
        return -1;
	}
...
int send_recv2_test(int n, int k)
{
    ...
    for (int i = 0; i < k; i++) {
        if (rank == 0) {
            t_sendrecv += send_recv_msg(n, 1, send_buf, recv_buf);
        } else if (rank == 1) {
            t_sendrecv += send_recv_msg(n, 0, send_buf, recv_buf);
        }
    }
    if (rank == 0) {
        printf("msg length: %d\n", n);
        printf("average send-recieve time from 0 to 1 (us): %.2lf\n", t_sendrecv * M / k);
    } else if (rank == 1) {
        printf("average send-recieve time from 1 to 0 (us): %.2lf\n", t_sendrecv * M / k);
    }
    free(send_buf);
    free(recv_buf); 
}
...
int main(int argc, char *argv[])
{
    ...
    send_recv2_test(200, 1000);
    send_recv2_test(500, 1000);
    send_recv2_test(1000, 500);
    send_recv2_test(2000, 200);
    send_recv2_test(5000, 200);
    send_recv2_test(10000, 100);
    MPI_Finalize();
